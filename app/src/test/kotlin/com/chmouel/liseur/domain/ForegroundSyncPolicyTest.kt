...
class ForegroundSyncPolicyTest {
    ...
    fun `a kosync partner makes sync due even when the server cannot sync itself`() {
        val custom = server(kind = ServerKind.CUSTOM, koboToken = null, syncedAt = null)
        assertEquals(false, shouldSyncOnForeground(custom, now))
        assertEquals(
            true,
            shouldSyncOnForeground(custom, now, kosync = kosync(syncedAt = null)),
        )
    }

    @Test
    fun `a kosync partner that synced minutes ago is left alone`() {
        val custom = server(kind = ServerKind.CUSTOM, syncedAt = null)
        assertEquals(
            false,
            shouldSyncOnForeground(custom, now, kosync = kosync(syncedAt = now - 5 * 60 * 1000)),
        )
    }

    @Test
    fun `either partner being stale is enough to ask`() {
        val fresh = server(kind = ServerKind.CUSTOM, syncedAt = now - 5 * 60 * 1000)
        assertEquals(
            true,
