...
| Position sync | `GET`/`POST /api/v1/books/files/{fileId}/progress`, `PATCH /api/v1/books/{id}/status` | Not implemented | Phase 1 persists foreign CFIs with guarded file ownership and reads selected-file `/epub/{bookId}/info` for a package cross-check. It cannot restore or generate a CFI. `syncAbility` remains `NONE`: a percentage-only write would clear the server's exact position |
