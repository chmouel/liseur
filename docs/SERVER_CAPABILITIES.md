...
| Position and status sync | `GET`/`POST /api/v1/books/files/{fileId}/progress`, `GET /api/v1/books/{id}`, `PATCH /api/v1/books/{id}/status` | Exact EPUB position sync enabled; status sync implemented but gated | Position sync retains exact CFIs and selected-file identity. Status uses a separate agreement and one-shot status-only PATCH. `syncAbility = EXACT` describes positions; status remains disabled pending live independent-client acceptance |
